...
public interface IAuthService
{
    ...
    Task<bool> ClaimOwnershipAsync(JwtSecurityToken? sourceToken, JwtSecurityToken destinationToken);
    Task<bool> RegisterUserAsync(RegisterRequest request);
    Task<AccessOrRefreshResponse> RefreshAuthorizedUserAsync(JwtSecurityToken? accessToken, string refreshToken);
    Task<bool> RevokeUserTokenAsync(JwtSecurityToken accessToken, string refreshToken);
}
...
public class AuthService : IAuthService
{
    ...
    public async Task<bool> ClaimOwnershipAsync(JwtSecurityToken? sourceToken, JwtSecurityToken destinationToken)
    {
        var sourceUserId = UtilHelper.GetUserId(sourceToken);
        var destinationId = UtilHelper.GetUserId(destinationToken);
    ...
    public async Task<AccessOrRefreshResponse> RefreshAuthorizedUserAsync(JwtSecurityToken? accessToken, string refreshTokenValue)
    {
        var userId = UtilHelper.GetUserId(accessToken);
        var success = await RevokeUserTokenAsync(userId, refreshTokenValue);
        if (!success) return null;
        return await PrepareAccessResponseAsync(userId);
    }

    public async Task<bool> RevokeUserTokenAsync(JwtSecurityToken accessToken, string refreshToken)
    {
        var userId = UtilHelper.GetUserId(accessToken);
