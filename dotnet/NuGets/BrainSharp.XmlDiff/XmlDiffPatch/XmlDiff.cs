...
// Methods
    ...
    private void MatchIdenticalSubtrees()
    {
                ...
                targetNodesToExpand.Count > 0 )
        {
            ...
            for ( int i = 0; i < count; i++ )
            {
                ...
                if ( targetParentNode._bExpanded ) {
                    continue;
                }
                ...
                while ( curTargetNode != null )
                {
                    ...
                    // this is to prevent children of firstSourceNode from being further expanded in they are in sourceNodesToExpand queue; 
                    // (items cannot be removed from the middle of a queue)
                    MarkChildrenAsExpanded( firstSourceNode );  
                            ...
                            curSourceNode.NodeType != XmlDiffNodeType.ShrankNode )
                    {
                             ...
                             sourceUnmatchedNodes.Contains( curSourceNode.HashValue ) )
                        {
                            ...
                            // this is to prevent children of curSourceNode from being further expanded in they are in sourceNodesToExpand queue; 
                            // (items cannot be removed from the middle of a queue)
                            MarkChildrenAsExpanded( curSourceNode );  
                        }
            ...
            for ( int i = 0; i < count; i++ )
            {
                ...
                while ( curSourceNode != null )
                {
                    ...
                    if ( curSourceNode._bExpanded ||
                         curSourceNode is XmlDiffShrankNode ||
                    ...
                    // this is to prevent children of firstTargetNode from being further expanded in they are in targetNodesToExpand queue; 
                    // (items cannot be removed from the middle of a queue)
                    MarkChildrenAsExpanded( firstTargetNode );  
                            ...
                            curTargetNode.NodeType != XmlDiffNodeType.ShrankNode )
                    {
                            ...
                            targetUnmatchedNodes.Contains( curTargetNode.HashValue ) )
                        {
                            ...
                            // this is to prevent children of curTargetNode from being further expanded in they are in targetNodesToExpand queue; 
                            // (items cannot be removed from the middle of a queue)
                            MarkChildrenAsExpanded( curTargetNode );  
                        }
                        // no match -> end of interval
                        else {
                            break;
    ...
    // Finds first level with _bExpanded == false and changes it to true;
    // This is used to prevent descendant of 'node' from being further expanded in they are in {target,source}NodesToExpand queue; 
    // only one level with _bExpanded == false may be in the queue, so we don't need to walk the whole tree and set the flag
    private void MarkChildrenAsExpanded( XmlDiffNode node ) {
        if ( !node._bExpanded ) {
            node._bExpanded = true;
        }
        XmlDiffNode child;
        while ( ( child = node.FirstChildNode ) != null ) {
            if ( child._bExpanded ) {
                node = child;
                continue;
            }
            // found first child with _bExpanded == false -> walk siblings and set the flag
            do { 
                child._bExpanded = true;
